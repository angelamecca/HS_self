!!
!  nb : legge file con derivate e calcola la massa effettiva
!
!
!=============================================================================================================
!
...
!============================================  
  ...
  do 
     read(32,*,iostat=io)
     if(io.eq.0) count = count + 1
     if(io.gt.0) stop
     if(io.lt.0) exit
  end do
  write(6,*) 'ho letto ', count, 'derivate'
  rewind(32)

  read(32,*) 
  read(32,*) 
  do k = 1, count
     read(32,*) kk(k), Reh_de(k),Reh_de_er(k), Rep_de(k), Rep_de_er(k), Reh_dk(k),  Reh_dk_er(k), Rep_dk(k), Rep_dk_er(k), Rehf_dk(k)
     write(6,*) 'ci sono le deriv in ', k, kk(k), Reh_de(k),Reh_de_er(k), Rep_de(k), Rep_de_er(k), Reh_dk(k),  Reh_dk_er(k), Rep_dk(k), Rep_dk_er(k), Rehf_dk(k)
  end do
  close(32)
  
  
  name=trim(adjustl(dove))//"der_bis"//trim(filenum)//".dat"
  open(32,file=name)
  ...
  do k = 1, count
     write(32,7000) kk(k), Reh_de(k),Reh_de_er(k), Rep_de(k), Rep_de_er(k), Reh_dk(k),  Reh_dk_er(k), Rep_dk(k), Rep_dk_er(k), Rehf_dk
  end do

  if(count.lt.nk-2) then 
     do k = count+1, nk
        Ene(k) = spe(k)
        i0 = stencil/2 + 1
     ...
     do k = 1, nk
        ...
        do i = 1, stencil
           ...
           write(6,*) 'derivative E  hf', i, k, kk(k), Ene(k), kk(k), Etmp(i), ktmp(i)
           part = .true.
           call sigma_hf(r,nt,hr,ell,veff,ktmp(i),dens,deg,tmp_hf(i))
           write(27,*) i, k, Etmp(i), ktmp(i), tmp_eh(i),er_eh(i), tmp_ep(i),er_ep(i),tmp_kh(i),er_kh(i), tmp_kp(i),er_kp(i), tmp_hf(i)
           flush(27)
